...
function FixResultBlock({ outcome }: { outcome: FixOutcome }) {
  return (
    <div className={`mt-2 text-xs px-2.5 py-2 rounded-sm ${outcome.fixed_count > 0 ? 'bg-accent-green/10 text-accent-green' : 'bg-accent-amber/10 text-accent-amber'}`}>
      已修复 {outcome.fixed_count} 项，跳过 {outcome.skipped_count} 项
...
export function LintResult() {
  ...
  const handleDismiss = async (key: string) => {
    ...
    try {
      ...
      toast({ title: '忽略失败', description: err.response?.data?.detail || String(err), variant: 'error' })
    }
    setDismissing(null)
  }
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-5">
        ...
        <h2 className="text-title">体检报告</h2>
      </div>
      ...
      {!result?.error && result && (
        <div className="space-y-4">
          {presentTypes.length > 0 && (
            <div className="px-1">
              <p className="text-xs text-text-muted mb-1">本次体检发现的问题类型说明：</p>
              <ul className="space-y-0.5">
                {presentTypes.map((key) => (
                  <li key={key} className="text-xs text-text-muted">{ISSUE_TYPE_INFO[key]}</li>
                ))}
              </ul>
            </div>
          )}
          ...
          {!hasIssues && (
            <div className="card flex items-center gap-3 py-6">
              ...
              <p className="text-body text-text-secondary">Wiki 健康状况良好，未发现明显问题。</p>
            </div>
          )}

          <div className="card">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-subtitle">体检摘要</h3>
              <span className="text-mono text-text-primary text-sm">{summaryTotal} 项</span>
            </div>
          ...
          {result.conflicts?.length > 0 && (
            <IssueCard
              ...
              title={`矛盾检测 (${result.conflicts.length})`}
                ...
                return (
                  <div key={i} className="border-l-2 border-accent-red pl-3 py-1">
                    ...
                    <IssueMeta
                      ...
                      suggestion="人工核对两页的矛盾表述，统一事实后更新页面内容"
                    />
          ...
          {result.outdated_content?.length > 0 && (
            <IssueCard
              ...
              title={`过时内容 (${result.outdated_content.length})`}
          ...
          {result.missing_backlinks?.length > 0 && (
            <IssueCard
              ...
              title={`反向链接缺口 (${result.missing_backlinks.length})`}
              color="text-accent-amber"
              ...
              {result.missing_backlinks.map((b: any, i: number) => {
                ...
                return (
                  <div key={i} className="border-l-2 border-accent-amber pl-3 py-1">
                    ...
                    <IssueMeta
                      reason={`[[${b.from}]] 链接到 [[${b.to}]]，但 [[${b.to}]] 中缺少指回的链接`}
                      suggestion={`在 [[${b.to}]] 页面补充 [[${b.from}]] 链接（可点右上角「一键修复」自动完成）`}
                    />
                    {sug && <SuggestionBlock suggestion={sug} />}
          ...
          {result.conflict_annotations?.length > 0 && (
            <IssueCard
              ...
              title={`[!conflict] 标注 (${result.conflict_annotations.length})`}
              color="text-accent-amber"
            >
              {result.conflict_annotations.map((a: any, i: number) => {
                ...
                return (
                  <div key={i} className="border-l-2 border-accent-amber pl-3 py-1">
                    ...
                    <IssueMeta
                      reason="页面中存在人工标记的 [!conflict] 矛盾标注"
                      suggestion="核对标注处的矛盾内容，解决后手动移除标注"
                    />
                    {sug && <SuggestionBlock suggestion={sug} />}
                    <div className="flex gap-2 mt-2 items-center">
                      <button
          ...
          {result.missing_concepts?.length > 0 && (
            <IssueCard
              ...
              title={`缺失概念 (${result.missing_concepts.length})`}
              ...
              {result.missing_concepts.map((c: any, i: number) => {
                ...
                return (
                  <div key={i} className="border-l-2 border-wiki-concept pl-3 py-1">
                    ...
                    <IssueMeta
                      reason={`被 ${c.referenced_by.length} 个实体页引用，但尚无独立页面`}
                      suggestion="摄入或上传相关资料，经摄入流程创建该概念页"
                    />
                    {sug && <SuggestionBlock suggestion={sug} />}
                    {canEdit && (
                      <IssueActions
          ...
          {result.index_consistency && !result.index_consistency.consistent && (
            ...
            >
              <IssueMeta
                reason="_wiki_index.md 与实际页面不同步"
                suggestion="点击右上角「一键修复」重建索引"
              />
              <div className="space-y-2">
                {result.index_consistency.missing_from_index?.length > 0 && (
                  <div>
                    <p className="text-small text-text-secondary">缺失于 _wiki_index.md 的页面：</p>
                ...
                {result.index_consistency.extra_in_index?.length > 0 && (
                  <div>
                    <p className="text-small text-text-secondary">索引中多余的页面：</p>
                    <div className="flex flex-wrap gap-1.5 mt-1">
          ...
          {result.tag_consistency && !result.tag_consistency.consistent && (
            ...
            >
              <IssueMeta
                reason="_tag_registry.md 与页面实际使用的标签不同步"
                suggestion="点击右上角「一键修复」重建标签注册表"
              />
              <div className="space-y-2">
                {result.tag_consistency.missing_from_registry?.length > 0 && (
                  <div>
                    <p className="text-small text-text-secondary">未在 _tag_registry.md 中注册的标签：</p>
                    <div className="flex flex-wrap gap-1.5 mt-1">
                ...
                {result.tag_consistency.extra_in_registry?.length > 0 && (
                  <div>
                    <p className="text-small text-text-secondary">注册表中多余（未被使用）的标签：</p>
                    <div className="flex flex-wrap gap-1.5 mt-1">
          ...
          {result.orphan_pages?.length > 0 && (
            <IssueCard
              ...
              title={`孤立页面 (${result.orphan_pages.length})`}
            ...
            >
              {result.orphan_pages.map((slug: string) => (
                <div key={slug} className="border-l-2 border-accent-cyan pl-3 py-1">
                  ...
                  <IssueMeta
                    reason="未被任何其他页面链接，难以被发现"
                    suggestion={`在相关页面中添加 [[${slug}]] 链接，或确认该页面是否应保留`}
                  />
                  {canEdit && (
                    <IssueActions
          ...
          {result.reinforce_annotations?.length > 0 && (
            <IssueCard
              ...
              title={`[!reinforce] 标注 (${result.reinforce_annotations.length})`}
              ...
              {result.reinforce_annotations.map((a: any, i: number) => {
                ...
                return (
                  <div key={i} className="border-l-2 border-accent-green pl-3 py-1">
                    ...
                    <p className="text-small text-text-tertiary mt-0.5 font-mono">{a.context}</p>
                    <IssueMeta
                      reason="页面中存在人工标记的 [!reinforce] 待佐证标注"
                      suggestion="补充佐证资料确认该论断后，手动移除标注"
                    />
                    {sug && <SuggestionBlock suggestion={sug} />}
                    <div className="flex gap-2 mt-2 items-center">
                      <button
          ...
          {result.info_gaps?.length > 0 && (
            <IssueCard
              ...
              title={`信息缺口 (${result.info_gaps.length})`}
              ...
              {result.info_gaps.map((g: any, i: number) => {
                ...
                return (
                  <div key={i} className="border-l-2 border-default pl-3 py-1">
                    ...
                    <IssueMeta
                      reason="知识库中缺乏回答该问题的内容"
                      suggestion={g.suggested_source ? `按建议来源补充资料：${g.suggested_source}` : undefined}
                    />
