...
export function AuditLog() {
  ...
  const load = () => {
    ...
    api
      ...
      .catch(() => toast({ title: '加载操作日志失败', variant: 'error' }))
  ...
  const columns = [
    ...
    },
    {
      ...
      width: '20%',
      render: (log: AuditLog) => (
        <span className="text-xs text-text-tertiary">{formatTime(log.created_at)}</span>
      ),
    },
  ]
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="操作日志"
        description="查看系统中的所有操作记录，支持按日期、操作人、动作筛选。"
        icon={ScrollText}
      />

      <div className="flex flex-wrap items-end gap-3 p-3 border border-subtle rounded bg-surface">
        <div className="flex flex-col gap-1">
          <label className="text-[10px] uppercase tracking-wider text-text-tertiary">关键词</label>
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[10px] uppercase tracking-wider text-text-tertiary">操作人</label>
          <div className="relative">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[10px] uppercase tracking-wider text-text-tertiary">开始日期</label>
          <div className="relative">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[10px] uppercase tracking-wider text-text-tertiary">结束日期</label>
          <div className="relative">
            <Calendar size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-text-muted" strokeWidth={1.5} />
            <input
              type="date"
